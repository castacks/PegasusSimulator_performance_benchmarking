#!/usr/bin/env python
"""Benchmark 5: cube fall with Pegasus loaded, Full Warehouse scene, drone with PX4 MAVLink backend.

Combines the Full Warehouse scene (heaviest env) with the PX4 MAVLink 250 Hz
lockstep loop. This is the worst-case scenario and comparing against benchmarks
3 and 4 shows whether scene complexity + PX4 compound or are independent.
...
Run:
  ./python.sh benchmarking/5_cube_pegasus_complex_px4.py            # headless (default)
  ./python.sh benchmarking/5_cube_pegasus_complex_px4.py --no-headless
"""
import sys
import os
...
pg.load_environment(SIMULATION_ENVIRONMENTS["Full Warehouse"])
